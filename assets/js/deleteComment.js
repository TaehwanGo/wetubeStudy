...
const deleteCommentOnLocal = (commentId) => { // local에서 삭제(새로고침 없이 삭제되는 것 처럼 보이기 위해)
...
const deleteCommentFromDB = async (videoId, commentId) => {
    // fake delete 
...
function init() {
    ...
    for(let i = 0; i<ulArray.length; i++){ // 이벤트 리스너 추가
        ...
        if(ulArray[i].children[1]){ // form 태그가 있다면 (작성자가 작성했으면 있음)
